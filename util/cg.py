#################################################################################################
#  cg.py -
# cg method inversion to find model parameters for a description
...
def invert(F,m,m_apr,A,dd,alpha,num_res):
    """start cg iterations.  Uses regularized non-linear conjugate gradient method with line search for weighting of one."""
    count = 0;
    maxit = 5000;
    etol = 50e-9;
    iflag = 1;
    ...
    while iflag:
        ...
        if count == 0:
            ...
            beta = norm(l_n)**2/norm(l_prev)**2;
        l_tilda = l_n + beta*l_tilda_prev;
        #pdb.set_trace();
        k_n = dot(l_tilda,l_n)/(norm(dot(F,l_tilda))**2 + alpha*(norm(l_tilda)**2));
        m = m - k_n*l_tilda;
        l_tilda_prev = l_tilda;
        l_prev = l_n;
        (F,A) = calc(ll,t,m,num_res);  #this is the part that is custom made for this problem for more general porting change this line...
        e_2 = norm(A-dd);
        delt_e = abs(e_i-e_2);
        if e_2 > e_i:
            iflag = 0;
